---
id: quickbooks
title: CERP QuickBooks Migrator
sidebar_label: CERP QuickBooks Migrator
---
...
1. Go to Inuit Developer Portal
...
#### Existing Chart of Accounts
    ...
    Upon creation of a Company CERP creates a chart of accounts for that company, these accounts will be kept.
...
#### Account Naming
    ...
    **Note**: CERP also encodes account names with Company abbreviation. Taking this into account `Job Expense` will become  `Job Expense - QB - AZ` (assuming `AZ` is the company abbreviation).
...
#### Group Accounts
    ...
    QuickBooks allows transactions on group accounts, which is not allowed in CERP, to handle this, every group account will have a child with a hyphenated name.
...
#### Name Collisions
    ...
    QuickBooks allows multiple accounts to have the same name, which is not allowed in CERP, to handle this, every duplicate account will have a hyphenated name.
...
#### Discount and Markup
    ...
    QuickBooks uses special accounts for both Markup and Discount, CERP doesn't handle the discount expense and markup this way, instead, all Item's will see the change in their Income accounts.
...
#### Round off
    ...
    CERP uses different rounding method than QuickBooks, because of this, in Invoices with Tax and with a currency different than company currency, Sales Invoice will have different grand total than that of the QuickBooks Invoice.
